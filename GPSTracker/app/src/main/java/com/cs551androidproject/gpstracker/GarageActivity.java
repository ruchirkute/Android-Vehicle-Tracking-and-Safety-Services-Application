...
import com.cs551androidproject.gpstracker.Database.GarageController;
...
public class GarageActivity extends FragmentActivity {
    ...
    List<LocationObject> garageLocationList;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        int recordCount = new GarageController(this).count();
        if (recordCount > 0){
            garageLocationList=readRecords();
        }else{
            boolean cretedSuccessfully = new GarageController(getApplicationContext()).create();
            garageLocationList=readRecords();
        }
    ...
    public List<LocationObject> readRecords(){
        ...
        List<LocationObject> locations = new GarageController(this).read();
        if(locations.size()>0){
            return locations;
        }
        else{
    ...
    private void setUpMap() {
        for (int i = 0; i < garageLocationList.size(); i++) {
            mMap.addMarker(new MarkerOptions().position(new LatLng(garageLocationList.get(i).getLat(), garageLocationList.get(i).getLng())).title(""+garageLocationList.get(i).getTitle()));
        }
